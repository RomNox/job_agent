...
class ApplicationPackageRequest(BaseModel):
    ...
    raw_text: str | None = Field(
        default=None,
        min_length=1,
        description="Raw job posting text to analyze before preparing the application package.",
    )
    ...
    def validate_job_input(self) -> "ApplicationPackageRequest":
...
class ApplicationPackageChecklist(BaseModel):
...
class ApplicationPackage(BaseModel):
    ...
    cv_tailoring: CVTailoringResponse
    checklist: list[str] = Field(default_factory=list)
    warnings: list[str] = Field(default_factory=list)
